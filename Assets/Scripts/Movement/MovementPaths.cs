...
public class MovementPaths {
	...
	public static List<Vector3> CreateSnakePath(Vector3 startingPos,
			Vector2 levelWidth,
			int levels = 3,
			float distanceBetweenLevels = 2,
			bool rightToLeft = false) {

		List<Vector3> pathList = new List<Vector3>();
		distanceBetweenLevels *= -1;
		...
		if (new Vector3(levelWidth.x, startingPos.y) != startingPos) {
			pathList.Add(startingPos);
		}

		for (int i = 0; i < levels; i++) {
			pathList.Add(new Vector3(levelWidth.x, startingPos.y + i * distanceBetweenLevels));
			pathList.Add(new Vector3(levelWidth.y, startingPos.y + i * distanceBetweenLevels));
			pathList.Add(new Vector3(levelWidth.y, startingPos.y + i * distanceBetweenLevels + distanceBetweenLevels / 2));
			pathList.Add(new Vector3(levelWidth.x, startingPos.y + i * distanceBetweenLevels + distanceBetweenLevels / 2));
		}
